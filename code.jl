...
function doesit(gs)
    ...
    for g in first(gs, 40)
        ...
        (cur_pos, efound) = update_cur_pos!(cur_pos, as)#, last_pos[4])
        # here e is relative still 

        # @info cur_pos
        if last_pos[4] + cur_pos[4] > 0
            cur_pos[4] = 0
            if isempty(cur_path)
                p = [last_pos, cur_pos]
        ...
        else # e[i-1] + e[i] <= 0 (NEW PATH)
            # @show 
            if efound
                cur_pos[4] = last_pos[4] + cur_pos[4]
            end
            if !isempty(cur_path)
                @show "adding cur path to all_pls" cur_path
                push!(all_ps, cur_path)
                cur_path = []
            end
        end

        @info i, (last_pos, cur_pos)
        cur_pos[4] = min(0, cur_pos[4])
